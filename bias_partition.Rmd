...
fit_clrlm <- joint %>%
  filter(expected > 0) %>%
  group_by(sample_id) %>%
  mutate(clr_error = metacal::clr(observed0 / exp_individuals)) %>%
  ungroup() %>%
...
boot_models <- boots %>% 
  purrr::map(function(x){ 
    x <- x %>%
      mutate(fit = map(splits, ~lm(center_elts(observed0 / exp_individuals) ~ 0 + taxon, data = .)), 
...
ggplot(boot_aug, aes(x=exp_individuals, y=observed, colour=taxon)) +
