...
#create recipe
lm_rec <- model_dat %>%
  recipe(bias ~ exoskeleton + biomass_mm3+ surface_mm2 +  primerdist + `G|C`, data=model_dat) %>% #surface_mm2 +
...
extraction_bias_rec <- model_dat %>%
  recipe(bias ~ exoskeleton + biomass_mm3 + surface_mm2, data=model_dat) %>% #surface_mm2 +
  step_zv(all_numeric(), -all_outcomes()) %>%
  #step_corr(all_numeric(), -all_outcomes(), threshold = 0.9) %>%
  step_lincomb(all_numeric(), -all_outcomes()) %>%
  step_dummy(all_nominal(), one_hot=TRUE) %>%
...
pcr_bias_rec <- model_dat %>%
  recipe(bias ~ primerdist + `G|C`, data=model_dat) %>% #surface_mm2 +
  step_zv(all_numeric(), -all_outcomes()) %>%
  #step_corr(all_numeric(), -all_outcomes(), threshold = 0.9) %>%
  step_lincomb(all_numeric(), -all_outcomes()) %>%
  step_dummy(all_nominal(), one_hot=TRUE) %>%
...
seq_bias_rec <- model_dat %>%
  recipe(bias ~ `G|C`, data=model_dat) %>% #surface_mm2 +
  step_zv(all_numeric(), -all_outcomes()) %>%
  #step_corr(all_numeric(), -all_outcomes(), threshold = 0.9) %>%
  step_lincomb(all_numeric(), -all_outcomes()) %>%
  step_dummy(all_nominal(), one_hot=TRUE) %>%
...
partition_boot_fit <- model_dat %>%
  filter(!step == "seq") %>%
  mutate(recipe_type = step) %>%
  group_by(step, mixture_type, id)  %>%  #id
  nest() %>%
  mutate(models = purrr::map(data, function(y){
      # fit seperate models
      if(unique(y$recipe_type) == "all"){
        model_workflow<- workflow() %>%
            add_model(lm_mod) %>%
            add_recipe(lm_rec)
      } else if(unique(y$recipe_type) == "extract"){
        model_workflow<- workflow() %>%
            add_model(lm_mod) %>%
            add_recipe(extraction_bias_rec)
      } else if(unique(y$recipe_type) == "PCR"){
        model_workflow<- workflow() %>%
            add_model(lm_mod) %>%
            add_recipe(pcr_bias_rec)
      } else if(unique(y$recipe_type) == "seq"){
         model_workflow<- workflow() %>%
            add_model(lm_mod) %>%
            add_recipe(seq_bias_rec)
      }
...
gg.bias_traits <- partition_boot_pctl %>%
  filter(!(term == "(Intercept)" & step == "PCR")) %>% 
  ...
  #stat_summary(fun = mean, size=0.2, colour="black")+
...
# Option with all models fit
  
partition_boot_fit2 <- model_dat %>%
    filter(!step == "seq") %>%
...
partition_boot_pctl2 <- partition_boot_fit2 %>% 
  drop_na() %>%
  group_by(term, step, mixture_type) %>% 
  dplyr::do(rsample:::pctl_single(.$estimate, alpha = 0.05)) %>%
  dplyr::ungroup()

gg.bias_traits2 <- partition_boot_pctl2 %>%
  drop_na() %>%
  dplyr::mutate(term = term %>%
                  #str_replace("\\(Intercept\\)", "GC%") %>%
                  str_replace("\\(Intercept\\)", "Medium exoskeleton") %>%
...
#Save figure
pdf(file="figs/fig3_trait_effects_allmodels.pdf", width = 8, height = 5)
  plot(gg.bias_traits2)
try(dev.off(), silent=TRUE)
