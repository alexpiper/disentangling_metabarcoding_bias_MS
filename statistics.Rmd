...
ps_data <- ps0 %>%
  ...
  mutate(taxon = taxon %>% str_replace("nr.dimidiatus", "truncatus")) %>%
...
names(seqs) <- names(seqs) %>% str_replace("Carpophilus_nr.dimidiatus", "Carpophilus_truncatus")
...
fit_complm <- joint %>%
  ...
  group_by(mixture_type, target_subfragment)  %>%
  nest() %>%
  mutate(lm_obj = map(data, ~fit(lm_spec, err_individuals ~ 0 + taxon, data = .)),
           pred = map2(lm_obj, data, function(.model, .data) predict(.model, .data)),
           coef_info = map(lm_obj, tidy)
    )
...
Alternatively we can use the fido package from Justin silverman 
...
partition_fit <- error_preds %>%
  ...
  nest() %>%
  mutate(wf_fit = map(data, function(x){
    df <- x
...
# Convert these to stat_distributions as well
