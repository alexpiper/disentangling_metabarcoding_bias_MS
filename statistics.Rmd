...
set.seed(606)
boots <- purrr::map(lm_dat, bootstraps, times=10, apparant=TRUE)
...
extraction_bias_rec <- model_dat %>%
  recipe(bias ~ exoskeleton + biomass_mm3 , data=model_dat) %>% #surface_mm2 +
  step_zv(all_numeric(), -all_outcomes()) %>%
  #step_corr(all_numeric(), -all_outcomes(), threshold = 0.9) %>%
  step_lincomb(all_numeric(), -all_outcomes()) %>%
  step_dummy(all_nominal(), one_hot=TRUE) %>%
...
gg.bias_traits <- partition_boot_fit %>%
  ...
  scale_y_continuous(trans=pseudo_log_trans(0.5), breaks=c(-25, -5, -2, -1, 0, 
                                                           1, 2, 5, 25))+
...
gg.bias_traits2 <- partition_boot_fit2 %>%
  drop_na() %>%
  dplyr::mutate(term = term %>%
                  #str_replace("\\(Intercept\\)", "GC%") %>%
                  str_replace("\\(Intercept\\)", "Medium exoskeleton") %>%
