...
# Could i include this in a formula
# For instance a different dummy variable for each type of community
# Then get the coefficients for each
...
fit_complm2 <- joint %>%
  ...
  nest() %>%
  mutate(lm_obj = map(data, ~fit(lm_spec, err_individuals ~ 0 + taxon * mixture_type, data = .)),
           pred = map2(lm_obj, data, function(.model, .data) predict(.model, .data)),
           coef_info = map(lm_obj, tidy)
    )

# Could i include this in a formula as an interaction effect
# For instance a different dummy variable for each type of community
# Then get the coefficients for each
...
boot_models <- boots %>% 
  purrr::map(function(x){ 
    x <- x %>%
      mutate(lm_obj = map(splits, ~lm(err_individuals ~ 0 + taxon, data = .)), #why cant i use lm spec here?
